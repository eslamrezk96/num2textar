...
CURRENCY_KWD = [("دينار", "ديناران", "دينارات", "ديناراً"),
                ("فلس", "فلسان", "فلس", "فلس")]
CURRENCY_IRQ = [("دينار", "ديناران", "دينارات", "ديناراً"),
                ("فلس", "فلسان", "فلس", "فلس")]
CURRENCY_AED = [("درهم", "درهمان", "دراهم", "درهماً"),
                ("فلس", "فلسان", "فلس", "فلس")]

ARABIC_ONES = ["", "واحد", "اثنان", "ثلاثة", "أربعة", "خمسة", "ستة", "سبعة", "ثمانية",
...
def get_currency(currency_code):
    if currency_code == 'EGP':
        return CURRENCY_EGP
    elif currency_code == 'IQD':
        return CURRENCY_IRQ
    elif currency_code == 'AED':
        return CURRENCY_AED
    elif currency_code == 'KWD':
        return CURRENCY_KWD
            ...
            if tens < 20:
                if tens == 2 and int(hundreds) == 0 and group_level > 0:
                    ...
                    if tens == 1 and group_level > 0 and hundreds == 0:
                        ret_val += ""
                    elif (tens == 1 or tens == 2) and (group_level == 0 or group_level == -1) and \
...
print(Num2Word_AR().convert(5544444444444, 'EGP'))
